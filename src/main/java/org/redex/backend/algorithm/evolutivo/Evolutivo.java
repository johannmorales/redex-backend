...
public class Evolutivo implements Algoritmo {
    ...
    private TreeMultiset<Cromosoma> initialize(Oficina ofiOrigen, Oficina ofiDestino, ZonedDateTime current, Paquete paquete) {
    ...
    private List<VueloAgendado> buildRandomPath(Oficina ofiOrigen, Oficina ofiDestino, ZonedDateTime current) {
    ...
    private List<VueloAgendado> buildRandomLimitedPath(Oficina ofiOrigen, Oficina ofiDestino, ZonedDateTime current, ZonedDateTime limit) {
        List<VueloAgendado> posibles = gestorAlgoritmo.obtenerValidos(ofiOrigen, current)
                .stream()
    ...
    private void fitness(Cromosoma cromosome, Paquete paquete) {
        ...
        ZonedDateTime fin = cromosome.getGenes().get(cromosome.getGenes().size() - 1).getVueloAgendado().getFechaInicio();
